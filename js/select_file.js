...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "SelectFileFromFolder") {
            const onNodeCreated = nodeType.prototype.onNodeCreated;
            nodeType.prototype.onNodeCreated = function () {
                onNodeCreated?.apply(this, arguments);

                const folderWidget = this.widgets.find((w) => w.name === "folder_path");
                const extensionsWidget = this.widgets.find((w) => w.name === "extensions");
                const filterTypeWidget = this.widgets.find((w) => w.name === "filter_type");
                const filterTextWidget = this.widgets.find((w) => w.name === "filter_text");
