...
app.registerExtension({
    ...
    async nodeCreated(node, app) {
        if (node.comfyClass === "SelectFromList") {
            const listDataWidget = node.widgets.find((w) => w.name === "list_data");
            const inputModeWidget = node.widgets.find((w) => w.name === "input_mode");
            const delimiterWidget = node.widgets.find((w) => w.name === "delimiter");
            const selectedValueWidget = node.widgets.find((w) => w.name === "selected_value");
